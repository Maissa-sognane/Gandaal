            ...
            <div class="card text-white bg-primary mb-3" style="max-width: 18rem;">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-around flex-wrap">
                        <a href="{{route('editer_niveaux', ['id'=>$niveau->id])}}"><button class="btn btn-secondary">editer</button></a>
                        <a href="{{route('voir_classes', ['filiere'=>$filieres->id,'id'=>$niveau->id])}}"><button class="btn btn-success">Classes</button></a>
                    <form action="niveaux/{{$niveau->id}}" method="post">
                        @csrf
                        @method('delete')
                         <a href="{{route('supprimer', ['filiere'=>$filieres->id, 'id'=>$niveau->id])}}" name="delete"><button class="btn btn-danger">supprimer</button></a>
                    </form>
                    </div>
                </div>
            </div>
